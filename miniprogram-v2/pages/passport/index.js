...
Page({
  ...
  edit() { this.setData({ editing: true }, () => setTabBarHidden(this, true)); },
  closeEdit() { this.setData({ editing: false }, () => setTabBarHidden(this, false)); },
  async saveProfile(event) { this.setData({ submitting: true }); try { const user = await userService.update(sessionStore.state.userId, event.detail); sessionStore.updateUser(user); this.setData({ user, avatarInitial: String(user.nickname || user.name || "我").slice(0, 1), editing: false }, () => setTabBarHidden(this, false)); wx.showToast({ title: "资料已更新", icon: "success" }); } catch (error) { showError(error, "资料保存失败"); } finally { this.setData({ submitting: false }); } },
