...
Page({
  data: { loading: true, error: "", user: {}, initial: "我", memory: { total: 0, max_capacity: 50 }, counts: { profile: 0, goal: 0, action: 0, fact: 0 }, dashboard: {}, report: null, reportDate: "近期", strengths: [], risks: [], nextAction: "继续完成当前阶段任务", nextMeta: "来自行动计划", refreshedLabel: "" },
  ...
  async load() {
    ...
    try {
      ...
      const strengths = report && report.strengths && report.strengths.length ? report.strengths.slice(0, 2) : [];
      const risks = report && report.risks && report.risks.length ? report.risks.slice(0, 2) : [];
