...
Page({
  data: { sessionId: "", loading: true, error: "", report: {}, progress: {}, selectedPhase: {}, syncVisible: false, syncing: false, syncedKeys: [], createdLabel: "近期", reportStrength: "表达清晰", reportRisk: "缺少真实岗位体验" },
  onLoad(options) { if (!requireSession()) return; this.setData({ sessionId: options.sessionId || "" }); this.load(); },
  back() { wx.navigateBack(); },
  async load() { this.setData({ loading: true, error: "" }); try { const pair = await Promise.all([growthService.report(this.data.sessionId), todayService.progress(sessionStore.state.userId, this.data.sessionId)]); const report = normalizeReport(pair[0]); const progress = normalizeProgress(pair[1]); const syncedKeys = progress.phases.map((item) => item.key); report.phases = report.phases.map((phase) => Object.assign({}, phase, { synced: syncedKeys.indexOf(phase.key) >= 0 })); this.setData({ loading: false, report, progress, syncedKeys, selectedPhase: report.phases[0] || {}, createdLabel: report.createdAt ? String(report.createdAt).slice(0, 10) : "近期", reportStrength: (report.strengths && report.strengths[0]) || "表达清晰", reportRisk: (report.risks && report.risks[0]) || "缺少真实岗位体验" }); growthStore.set("report", report); growthStore.set("progress", progress); } catch (error) { this.setData({ loading: false, error: error.message || "报告加载失败" }); } },
