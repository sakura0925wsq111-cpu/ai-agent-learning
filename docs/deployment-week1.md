...
微信小程序根据微信版本自动选择 `Ver3 - 副本 (5)/config/env.js` 中的地址：开发版使用本地地址，体验版使用 test 地址，正式版使用 release 地址。体验/正式版部署前必须替换示例 HTTPS 域名，并在微信公众平台配置 request/uploadFile 合法域名。开发者可通过微信存储键 `API_BASE_URL` 临时覆盖开发版地址。
