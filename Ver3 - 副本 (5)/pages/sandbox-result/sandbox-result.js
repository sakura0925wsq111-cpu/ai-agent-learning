...
Page({
  ...
  async onLoad(options) {
    ...
    const { session_id, sandbox_session_id } = options;
    this.setData({
      statusBarHeight: info.statusBarHeight,
      sessionId: session_id || "",
      sandboxSessionId: sandbox_session_id || session_id || ""
    });
    this.loadPaths();
    this.loadResult();
  },
  ...
  async loadPaths() {
    try {
      ...
      if (res && res.paths && res.paths.length) {
        ...
        this.setData({
          directions: res.paths.map(p => ({
            type: p.type, name: p.name,
            icon: icons[p.type] || "/images/icon-postgrad.png",
            color: colors[p.type] || "#4A90D9",
            bgColor: bgs[p.type] || "#E6F2FF",
            description: p.description || "",
            matchScore: 0, recommended: false
          }))
        });
      }
    } catch (err) { console.error("加载路径失败:", err); }
  },
  ...
  async loadResult() {
    ...
    try {
      ...
      this.setData({
        ...
        insights: res.insights || this.getDefaultInsights()
      });
      this.updateDirections(res.matches || []);
      if (res.recommendation) this.setRecommendation(res.recommendation);
    } catch (err) {
  ...
  getDefaultInsights() {
    return [
      { label: "当前状态", value: "待分析", color: "#4A90D9" },
      { label: "核心优势", value: "待分析", color: "#52C41A" },
      { label: "主要困惑", value: "待分析", color: "#FA8C16" },
      { label: "建议行动", value: "待分析", color: "#722ED1" }
    ];
  },
  ...
  setRecommendation(rec) {
    const d = this.data.directions.find(d => d.type === rec.type);
    if (!d) return;
    this.setData({ recommendation: { ...d, reason: rec.reason || "基于你的背景，" + d.name + "是最适合你的发展方向" } });
  },
  ...
  async reanalyze() {
    ...
    if (!c.confirm) return;
    wx.showModal({
      title: "想调整什么？", content: "告诉AI你想调整的方向或方面", editable: true,
      placeholderText: "比如：我更关注就业方向而非考研",
      success: async (ir) => {
        ...
        try {
          await app.request({
            ...
            data: { session_id: this.data.sandboxSessionId, user_id: this.getUserId(), correction: ir.content }
          });
          wx.hideLoading();
          wx.redirectTo({ url: "/pages/chatroom/chatroom?mode=sandbox&session_id=" + this.data.sandboxSessionId + "&corrected=1" });
